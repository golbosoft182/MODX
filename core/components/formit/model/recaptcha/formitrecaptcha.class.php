...
<noscript><div>
	    ...
	    */
        ...
        /* the new code */
	    $html = '<script src="https://www.google.com/recaptcha/api.js" async defer></script><div class="g-recaptcha" data-sitekey="'.$this->config[FormItReCaptcha::OPT_PUBLIC_KEY].'"></div>';
        /* the new code */
    ...
    public function checkAnswer ($remoteIp, $challenge, $responseField, $extraParams = array()) {
        ...
        */
        ...
        $secret_new = $this->config[FormItReCaptcha::OPT_PRIVATE_KEY];
        $captcha_new = $_POST["g-recaptcha-response"];
        $rsp = file_get_contents("https://www.google.com/recaptcha/api/siteverify?secret=$secret_new&response=$captcha_new");
        $arr = json_decode($rsp, true);
        
        if($arr["success"]) $response->is_valid = true;
        else $response->is_valid = false;
        /* the new code */
